...
async def _worker(
    ...
    while not stop_event.is_set():
        ...
        try:
            resp = await client.post(endpoint, json=_PAYLOAD)
...
async def _run(url: str, duration: int, concurrency: int) -> None:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        tasks = [
            asyncio.create_task(_worker(client, endpoint, stop_event, results, errors))
...
def main(
    ...
    asyncio.run(_run(url, duration, concurrency))
